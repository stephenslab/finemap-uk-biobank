...
# PLINK command
# plink2 --linear hide-covar sex omit-ref --pfile height.chr1.imputed.2000 --covar covar.height.txt --pheno pheno.height.txt --vif 100 --out height.chr1.imputed.2000.plink

# PLINK computes vif for each column of Z, which treat factor levels separately. Some vif for one level of factor is very high.
# Using R, we can compute one vif for all factor levels. In this case, vif is close to 1.
