...
```{r CABLES1_bhat_nostandardize_results, warning=FALSE, fig.align='center', fig.width=45, fig.height=15}
...
The correlation between rs7235010 and rs12327253 is `r R[1293,1281]`. The average correlation between SNPs in CS1 and CS2 is
```{r}
round(mean(abs(R[mod_bhat$sets$cs$L1, mod_bhat$sets$cs$L2])), 4)
```

Zoom in plot:
```{r CABLES1_bhat_nostandardize_results_CSszoomin, warning=FALSE, fig.align='center', fig.height=8, fig.width=10}
...
```{r CABLES1_bhat_nostandardize_CS1_2, warning=FALSE, fig.align='center', fig.height=8, fig.width=20}
...
```{r CABLES1_bhat_nostandardize_CS2_1, warning=FALSE, fig.align='center', fig.height=8, fig.width=20}
...
In the following simulation, we treat rs7235010 and rs12327253 as true signals. The effect sizes are from estimated model. We use the fitted residual variance in the simulation. The response y is simulated from
$$
