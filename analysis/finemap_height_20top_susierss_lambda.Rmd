---
title: "Fine-mapping Height on More Regions (SuSiE-RSS with lambda)"
author: "Yuxin Zou"
date: "10/06/2019"
---
...
mod_rss = susie_rss(z = z, R = R, lambda=1e-04)
z.max = which.max(abs(z))
pip_rss = susie_plot_locuszoom(z, mod_rss, pos = ss.dat$pos$POS/1e6, chr = unique(ss.dat$pos$`#CHROM`), gene.pos.map = gene.pos.map, ld = R[z.max,], z.ref.name = names(z.max), title = paste0(region.name,' SuSiE RSS lambda=1e-4'))
p_rss = susie_plot_locuszoom(z, mod_rss, pos = ss.dat$pos$POS/1e6, chr = unique(ss.dat$pos$`#CHROM`), gene.pos.map = gene.pos.map, ld = R[z.max,], z.ref.name = names(z.max), title = paste0(region.name,' SuSiE RSS lambda=1e-4'), y.susie = 'p')
z_rss = susie_plot_locuszoom(z, mod_rss, pos = ss.dat$pos$POS/1e6, chr = unique(ss.dat$pos$`#CHROM`), gene.pos.map = gene.pos.map, ld = R[z.max,], z.ref.name = names(z.max), title = paste0(region.name,' SuSiE RSS lambda=1e-4'), y.susie = 'z')
