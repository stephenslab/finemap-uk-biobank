...
We extract SNPs on chr 3 from 140,800,000 to 141,800,000 with MAF > 0.01 and inputation score > 0.9. 
...
zcat /gpfs/data/pierce-lab/uk-biobank-genotypes/ukb_mfi_chr3_v3.txt.gz | awk '($6 < 0.01 || $8 < 0.9){print $2}' > ukb_mfi_chr3_v3_exclude_id.txt
...
There are 2896 variantes in the extracted file.
...
```
```{r}
lm.res = read.delim('../data/height.ZBTB38.plink2.height.glm.linear')
...
saveRDS(neale.choose, 'height.ZBTB38.neale.rds')
```

```{r}
neale.choose = readRDS('../data/height.ZBTB38.neale.rds')
...
mod = readRDS('../data/height.ZBTB38.susie.model.rds')
...
```{r fig.align='center', fig.width=30, fig.height=15}
mod_bhat.ld = susie_bhat(bhat = as.vector(betas), shat = as.vector(se), R = R, n = ss.dat$n, var_y = as.numeric(ss.dat$yty)/(ss.dat$n-1))
